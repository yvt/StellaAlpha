...
Q_SIGNALS:
    ...
    // NEW: requestReadyEx with response code. returned even when response is empty.
    void requestReadyEx(QByteArray networkReply, int code);
    ...
    void authenticatedStreamUpdated(QByteArray networkReply, int id);
...
private Q_SLOTS:
    ...
    void onAuthorizedRequestReplyReadyRead();
