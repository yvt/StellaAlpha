...
void KQOAuthManager::abortCurrentRequest(){
    Q_D(KQOAuthManager);
    ...
    d->currentRequest->disconnect(SIGNAL(error(QNetworkReply::NetworkError)));
...
void KQOAuthManager::executeRequest(KQOAuthRequest *request) {
    ...
    connect(d->networkManager, SIGNAL(sslErrors(QNetworkReply*,QList<QSslError>)),
...
void KQOAuthManager::executeAuthorizedRequest(KQOAuthRequest *request, int id) {
    ...
    if (request->httpMethod() == KQOAuthRequest::GET) {
        ...
        connect(reply, SIGNAL(readyRead()),
                this, SLOT(onAuthorizedRequestReplyReadyRead()));
...
void KQOAuthManager::onRequestReplyReceived( QNetworkReply *reply ) {
    ...
    Q_ASSERT(d->networkManager==sender());

    // avoid crash
    if(d->currentRequest==NULL)
        return;

    if(d->currentRequest!=reply){
        reply->deleteLater();;
        return;
    }
    ...
    */
    ...
    if(networkReply.isEmpty()) {
        //emit requestReadyEx(networkReply, (int)reply->error());
        Q_ASSERT(d->currentRequest==reply);
        d->currentRequest=NULL;
        reply->deleteLater();
        return;
    }
    ...
    if (d->error != KQOAuthManager::NoError) {
        ...
        emit requestReadyEx(networkReply, (respCode>=400)?respCode:((int)reply->error()));
        d->emitTokens();
        return;
    }
    ...
    emit requestReady(networkReply);
    emit requestReadyEx(networkReply,(respCode>=400)?respCode:0);
...
void KQOAuthManager::slotError(QNetworkReply::NetworkError error) {
    ...
    emit requestReadyEx(reply->readAll(), (int)error);
...
void KQOAuthManager::onAuthorizedRequestReplyReadyRead() {
    Q_D(KQOAuthManager);

    if (!d->r) {
        qWarning() << "Current KQOAuthRequest is not valid!";
        return;
    }
    ...
    while (reply->canReadLine()) {
        emit authenticatedStreamUpdated(reply->readLine(), id);
    }
}

void KQOAuthManager::sslErrors(QNetworkReply *rep, QList<QSslError>){
    // TODO: SSL error handling
    rep->ignoreSslErrors();
}
